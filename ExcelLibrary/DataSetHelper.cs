...
namespace ExcelLibrary
{
    ...
    public sealed class DataSetHelper
    {
        ...
        public static void CreateWorkbook(String filePath, DataSet dataset,string reportname,string reportdate)
        {
            ...
            foreach (DataTable dt in dataset.Tables)
            {
                ...
                worksheet.Cells[0, 7] = new Cell(reportname);
                worksheet.Cells[2, 7] = new Cell(reportdate);
                for (int i = 0; i < dt.Columns.Count; i++)
                {
                    // Add column header
                    worksheet.Cells[5, i] = new Cell(dt.Columns[i].ColumnName);

                    // Populate row data
                    for (int j = 0; j < dt.Rows.Count; j++)
                        worksheet.Cells[j + 6, i] = new Cell(dt.Rows[j][i].ToString());
                }
